...
- **App segregation & recovery confinement:** role `admin` cannot hold a session in this app (Admin Console only) — enforced in `signInAction`, `/auth/confirm`, `requireUser()` (bounces stale sessions via `/auth/signout?error=admin_console`), and the forgot-password/magic-link senders silently skip admin profiles. Recovery/invite sessions are confined by the `om-flow-gate` cookie (`lib/auth/flow-gate.ts` + proxy) to `/reset-password` / `/accept-invite`; `resetPasswordAction` signs out on BOTH failure (`?error=reset_failed`) and success (`?message=password_updated` — fresh sign-in required). The admin app inversely blocks `user`/`manager` (`error=ops_app`) and sends ops-role invite/reset links to this app via `NEXT_PUBLIC_OPS_APP_URL`.
