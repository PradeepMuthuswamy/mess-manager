...
- **Auth:** Better Auth (`better-auth`) with MongoDB adapter (`@better-auth/mongo-adapter`), plugins for bearer tokens (`bearer()`), two-factor authentication (`twoFactor()` for MFA/TOTP), and Next.js cookies (`nextCookies()`). Sessions are refreshed in `proxy.ts` using `auth.api.getSession({ headers: request.headers })`. Additional user fields (`role`, `unit_id`, `home_unit_id`, `capabilities`, `rank`, `service_number`, `status`) are managed in MongoDB collections and resolved on the server via `getCurrentUser()` in `lib/auth/get-current-user.ts`. Client components use `authClient` (`lib/auth/auth-client.ts`).
- **Auth emails:** all auth emails (recovery, invite, magic link) are sent by the app via Resend (`lib/email/resend.ts`) with first-party links `${NEXT_PUBLIC_SITE_URL}/auth/confirm?token=…&type=…&next=…` built by `buildAuthConfirmLink()` (`lib/auth/email-links.ts`) backed by the `verification` collection in MongoDB; `app/auth/confirm/route.ts` verifies the token and sets session cookies.
- **App segregation & recovery confinement:** roles `admin` and `super_admin` cannot hold a session in this app (Admin Console only) — enforced in `signInAction`, `/auth/confirm`, `proxy.ts`, `requireUser()` (bounces stale sessions via `/auth/signout?error=admin_console`), and the forgot-password/magic-link senders silently skip admin profiles. Recovery/invite sessions are confined by the `om-flow-gate` cookie (`lib/auth/flow-gate.ts` + proxy) to `/reset-password` / `/accept-invite`; `resetPasswordAction` signs out on BOTH failure (`?error=reset_failed`) and success (`?message=password_updated` — fresh sign-in required). The admin app inversely blocks `user`/`manager` (`error=ops_app`) and sends ops-role invite/reset links to this app via `NEXT_PUBLIC_OPS_APP_URL`.
...
- For sensitive operations, enforce MFA (`aal2`) where required by checking `user.aal === 'aal2'`.
