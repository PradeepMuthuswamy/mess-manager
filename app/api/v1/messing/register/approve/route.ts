...
import { approveDailyRegisterSchema } from '@/lib/schemas';
import { checkRateLimit } from '@/lib/api/rate-limit';
import { getIdempotencyKey, tryReplay, storeResponse } from '@/lib/api/idempotency';
import { approveDailyRegisterAction } from '@/lib/messing/actions';
...
export const POST = withRoute(async (req: NextRequest) => {
  ...
  const parsed = approveDailyRegisterSchema.safeParse(JSON.parse(bodyText || 'null'));
  ...
  const res = await approveDailyRegisterAction(parsed.data);
