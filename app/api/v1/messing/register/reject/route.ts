...
import { rejectDailyRegisterSchema } from '@/lib/schemas';
import { checkRateLimit } from '@/lib/api/rate-limit';
import { getIdempotencyKey, tryReplay, storeResponse } from '@/lib/api/idempotency';
import { rejectDailyRegisterAction } from '@/lib/messing/actions';
...
export const POST = withRoute(async (req: NextRequest) => {
  ...
  const parsed = rejectDailyRegisterSchema.safeParse(JSON.parse(bodyText || 'null'));
  ...
  const res = await rejectDailyRegisterAction(parsed.data);
