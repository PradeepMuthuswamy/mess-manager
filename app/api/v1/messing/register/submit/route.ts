...
import { submitDailyRegisterSchema } from '@/lib/schemas';
import { checkRateLimit } from '@/lib/api/rate-limit';
import { getIdempotencyKey, tryReplay, storeResponse } from '@/lib/api/idempotency';
import { submitDailyRegisterAction } from '@/lib/messing/actions';
...
export const POST = withRoute(async (req: NextRequest) => {
  ...
  const parsed = submitDailyRegisterSchema.safeParse(JSON.parse(bodyText || 'null'));
  ...
  const res = await submitDailyRegisterAction(parsed.data);
