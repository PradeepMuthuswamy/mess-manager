...
import { dailyKitchenExpenditureSchema } from '@/lib/schemas';
import { checkRateLimit } from '@/lib/api/rate-limit';
import { getIdempotencyKey, tryReplay, storeResponse } from '@/lib/api/idempotency';
import { saveKitchenExpenditureAction } from '@/lib/messing/actions';
...
export const POST = withRoute(async (req: NextRequest) => {
  ...
  const parsed = dailyKitchenExpenditureSchema.safeParse(JSON.parse(bodyText || 'null'));
  ...
  const res = await saveKitchenExpenditureAction(parsed.data);
