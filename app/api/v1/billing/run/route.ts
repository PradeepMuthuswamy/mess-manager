...
import { runBillingSchema } from '@/lib/schemas';
import { checkRateLimit } from '@/lib/api/rate-limit';
import { getIdempotencyKey, tryReplay, storeResponse } from '@/lib/api/idempotency';
import { runMonthlyBillingAction } from '@/lib/billing/actions';
...
export const POST = withRoute(async (req: NextRequest) => {
  ...
  const parsed = runBillingSchema.safeParse(JSON.parse(bodyText || 'null'));
  ...
  const res = await runMonthlyBillingAction(parsed.data);
