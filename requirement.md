...
| **Ration Auto-Calculation Cron** | `ration_consumptions` is queried manually by date. | Automated cron job calculates daily scale draws for all onboarded messes at midnight. | Implement Supabase scheduled edge function / Next.js cron runner applying active scales to attendance. |
